...

{
   ...
   static
   function logIn($user = false, $pass = false, $email_authcode = false, $email_authcode_request = false)
   {
      global $CFG;
      $ip = API::getUserIp();
      ...
      curl_setopt($ch, CURLOPT_POSTFIELDS, array(
         ...
         'email_authcode_request' => $email_authcode_request
      ));
   ...
   {
      global $CFG;
      if (isset($query['User']['verifyLogin']['results'][0])) $result = $query['User']['verifyLogin']['results'][0];
      if (!empty($result['attempts']) && (empty($_SESSION['attempts']) || $result['attempts'] > $_SESSION['attempts'])) self::$attempts = $result['attempts'];
      else
      if (!empty($_SESSION['attempts'])) self::$attempts = $_SESSION['attempts'];
   ...
   {
      global $CFG;
      if (!self::$awaiting_token) return false;
   ...
   {
      global $CFG;
      API::add('User', 'sendSMS', array(
         $authy_id
      ));
      $query = API::send();
      $response = $query['User']['sendSMS']['results'][0];
      if (!$response || !is_array($response)) Errors::add(Lang::string('security-com-error'));
      elseif ($response['success'] == false) Errors::merge($response['errors']);
      else {
         return true;
      }
   }
